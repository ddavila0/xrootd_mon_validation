...
def request_vector(server, random_file, vector, vector_size, byte_range_size, appinfo):
    byte_vector, status = server.fetch_vector(random_file, vector, appinfo)
    if status == -1:
        print("### ERROR: Cannot open file: "+ random_file +" at server: "+server.server_url)
    else:
        assert(len(byte_vector) == vector_size)
        for byte_range in byte_vector:
            assert(len(byte_range) == byte_range_size)

def run_test(server, initial_job_id, num_rounds, num_workers, random_file, file_size, vector_size, byte_range_size, file_out):
    ...
    for i in range(0, num_rounds):
        ...
        for j in range(0, num_workers):
            ...
            for i in range(0, vector_size):
                start = random.randint(0,file_size-byte_range_size-1)
                end = start + byte_range_size
            ...
            my_dict[job_id_s]=[random_file, vector]
            p = Process(target=request_vector, args=(server, random_file, vector, vector_size, byte_range_size, appinfo))
            p.start()
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        print("format of tests_file: <total_vectors> <vectors_per_second> <vector_size> <byte_range_size> <initial_job_id> <file_out>")
        sys.exit(1)
    ...
    list_of_files = "vr_list_of_files.txt"
    #---------------------------------------------------------
    # Rabbit
    queue="xrd.wlcg-itb"
    remove = True
    ...
    for line in lines:
        ...
        byte_range_size = int(line.split()[3])
        initial_job_id = int(line.split()[4])
        file_base = line.split()[5]
        ...
        # Run test
        #print("running test: " + str(total_requests) +" "+ str(requests_per_second)+" "+ str(vector_size)+" "+ str(byte_range_size)+ " " + str(initial_job_id) + " " + file_base)
        request_dict = run_test(server, initial_job_id, num_rounds, num_workers, random_file, file_size, vector_size, byte_range_size, file_out)
        ...
        for key in request_dict:
            ...
            req_vector   = request_dict[key][1]
            req_bytes=0
            for byte_range in req_vector:
                req_bytes +=byte_range[1] - byte_range[0]
